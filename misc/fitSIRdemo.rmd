...
findSens(bombay2, trans.pars(fitP2), plot.it = TRUE)
...
findSens(bombay2, trans.pars(fitP.inc), plot.it = TRUE, incidence = TRUE)
...
count[1] #4
with(as.list(trans.pars(fitP.inc)),{beta * N * (1 -i0) * i0}) ## 4.35
```
...
fake.inc = data.frame(tvec, count = SIR.detsim(tvec, pars, incidence = TRUE))
...
findSens(fake.inc, trans.pars(fake.P), plot.it = TRUE, incidence = TRUE)
```

```{r}
(fake.P2 <- fitsir.optim(fake.inc, start = startfun(), plot.it=TRUE, incidence = TRUE))

findSens(fake.inc, trans.pars(fake.P2), plot.it = TRUE, incidence = TRUE)
