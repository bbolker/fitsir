...
load("stochsim_5.rda")  ## supplementary spline results, LHS data frame
names(res.spline2)[1] <- "mse.spline1" ## disambiguate
estpars <- res[,c("log.beta","log.gamma","log.N","logit.i")]
match.lhspars <- function(p) {
    tt <- trans.pars(p)
    ss <- summarize.pars(p)
    return(c(ss["R0"],tt["gamma"],ss[c("I0","N")]))
}
estpars2 <- t(apply(estpars,1,function(x) trans.pars))
source("stochsim_funs.R")
p <- lhs_df[1,]
p2 <- c(p[2:3],
...
plot(count~tvec,data=d)
f1 <- fitfun2(d)
summarize.pars(f1[2:5])
```
