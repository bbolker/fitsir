...
I found this parameter manually:

```{r SierraLeone}
pars.S <- c(log.beta = 0.52, log.gamma = 0.505, log.N = 13, logit.i = -14)
data = "SierraLeone"
case <- paste0("Cases_", data)
cumInc <- eboladf[,c("Day", case)]
cumInc <- cumInc[!is.na(cumInc[,2]),]
...
Cumulative incidence certainly looks great... What happens if we use these to for fitting?

```{r sfit}
tmpfun(data = "SierraLeone", start.pars = pars.S, plot.it = TRUE)
```

This works. I guess we needed a good starting parameter.
