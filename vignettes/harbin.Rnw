...
Overdispersion?
<<harbin_overd>>=
(ff2 <- fitsir(harbin, dist="quasipoisson", type="death", 
              tcol="week", icol="Deaths", method="BFGS"))
(ff3 <- fitsir(harbin, dist="nbinom", type="death", 
              tcol="week", icol="Deaths"))
(ff4 <- fitsir(harbin, dist="nbinom1", type="death", 
              tcol="week", icol="Deaths", hessian.opts=list(r=6)))
plot(ff2, level=0.95, col.traj="red", col.conf="red")
plot(ff3, level=0.95, add=TRUE, col.traj="blue", col.conf="blue")
plot(ff4, level=0.95, add=TRUE, col.traj="green", col.conf="green")
@
...
(bb3 <- fitsir(bombay2, dist="nbinom1", start=coef(bb1), hessian.opts=list(d=1e-5,r=6)))
@
